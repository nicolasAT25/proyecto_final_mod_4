...
def update_student(numero_documento, nombre, apellido, fecha_nacimiento, email, telefono):
    """Actualizar un estudiante existente."""
    try:
        conn = db_connect()
        if conn:
            query = "UPDATE estudiantes SET nombre = %s, apellido = %s, fecha_nacimiento = %s, email = %s, telefono = %s WHERE numero_documento = %s"
            values = (nombre, apellido, fecha_nacimiento, email, telefono, numero_documento)
            execute_query(conn, query, values)
            commit_changes(conn)
    finally:
        close_db_conn(conn)
